...
main <- fread('../data/phenotypes/ukb-raw/ukb45188.tab',
              select = c('f.eid', 'f.21022.0.0', 'f.31.0.0', 'f.22000.0.0', 'f.54.0.0'),
              data.table = FALSE)

## load list of withdrawn samples (as of 1 Feb 2021)
withdrawn <- fread('../data/sample-ids/withdrawals/w27960_20210201.csv', data.table = FALSE)

## load most recent imputed data sample file (downloaded 14 Jun 2021)
...
## join
covars <- merge(covars, anc_iid, by.x = 'f.eid', by.y = 'IID')
