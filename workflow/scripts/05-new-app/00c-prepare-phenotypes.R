...
phen_677925_cols <- fread("../data/phenotypes/ukb-raw/ukb677925.tab",
                   header = FALSE, nrows = 1, data.table = FALSE)
phen_677925_cols <- unname(unlist(phen_677925_cols))  # get vector
...
## process traits in dataset from new application

## list of traits available in phenotype table
phen_677925_ids_list <- strsplit(phen_677925_cols, split = ".", fixed = TRUE)
## get 2nd and 4th item from each element in list
phen_677925_ids <- sapply(phen_677925_ids_list, "[", 2)
phen_677925_array <- sapply(phen_677925_ids_list, "[", 4)

## initialise empty vector of traits which are not available in phenotype table
na_traits_677925 <- vector()
## initialise empty vector of traits with more than one array index
array_traits_677925 <- vector()
...
for (t in traits) {
    ...
    if (t %in% phen_677925_ids) { 
        cols <- which(phen_677925_ids == t)  # get column indices

        ## load data for these columns only
        phen_677925_t <- fread("../data/phenotypes/ukb-raw/ukb677925.tab",
                              select = c(1, cols), data.table = FALSE)
        ## keep only relevant IIDs
        phen_677925_t <- phen_677925_t[phen_677925_t$f.eid %in% all_sp_ids,]
        ## order by all-ids.tab file (matches order of original sample file)
        phen_677925_t <- phen_677925_t[match(all_sp_ids, phen_677925_t$f.eid),]
        ...
        if (t != 78 & any(phen_677925_t[, 2:ncol(phen_677925_t)] < 0, na.rm = TRUE)) { 
            stop(paste("phenotype", t, "has negative values.")) 
        }


        ## check that there is only one array index per instance (visit)
        if (identical(as.numeric(phen_677925_array[cols]), rep(0, length(cols)))) {

            ## get right-most value if there is more than one
            if (length(cols) > 1) {
                latest <- apply(phen_677925_t[, 2:ncol(phen_677925_t)], 1, get_right)
            } else {
                latest <- phen_677925_t[, 2]
            }
                
            ## add eid column
            latest_df <- data.frame(phen_677925_t$f.eid, latest)
            ...
            array_traits_677925 <- c(array_traits_677925, t)
        }
    } else {
        na_traits_677925 <- c(na_traits_677925, t)
    }
}


## all traits are available in this dataset
## > na_traits_677925
## logical(0)

## 4 traits with multiple array indices:
...
for (t in c(102, 4080, 4079)) {
    ...
    df <- data.frame(inst.1 = rowMeans(phen_677925_t[, 2:3], na.rm=TRUE),
                     inst.2 = rowMeans(phen_677925_t[, 4:5], na.rm=TRUE),
                     inst.3 = rowMeans(phen_677925_t[, 6:7], na.rm=TRUE),
                     inst.4 = rowMeans(phen_677925_t[, 8:9], na.rm=TRUE))
...
cols_399   <- which(phen_677925_ids == 399)  # get column indices

## load data for these columns only
phen_677925_t <- fread("../data/phenotypes/ukb-raw/ukb677925.tab",
                       select = c(1, cols_399), data.table = FALSE)
## keep only relevant IIDs
phen_677925_t <- phen_677925_t[phen_677925_t$f.eid %in% all_sp_ids,]
## order by all-ids.tab file (matches order of original sample file)
phen_677925_t <- phen_677925_t[match(all_sp_ids, phen_677925_t$f.eid),]
...
## check that there are no negative values indicating no response, etc
if (any(phen_677925_t[, -1] < 0, na.rm = TRUE)) {
...
df_399 <- data.frame(f.eid  = phen_677925_t$f.eid,
                     inst.1 = rowSums(phen_677925_t[, cols_399_1], na.rm=FALSE),
                     inst.2 = rowSums(phen_677925_t[, cols_399_2], na.rm=FALSE),
                     inst.3 = rowSums(phen_677925_t[, cols_399_3], na.rm=FALSE),
                     inst.4 = rowSums(phen_677925_t[, cols_399_4], na.rm=FALSE))
## keep most recent non-NA value
latest_399 <- apply(df_399[, 2:5], 1, get_right)
## add eid column
latest_df_399 <- data.frame(phen_677925_t$f.eid, latest_399)
