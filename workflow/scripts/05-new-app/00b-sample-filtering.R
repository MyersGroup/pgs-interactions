...
##
##       First create largest list of samples to consider in the
##       analysis (everyone who hasn't withdrawn and has all covars available).
##       We get 487255 individuals in total.
##
##       Now we add separate files for:
##       - females and males (264189 and 223066 resp.)
##       - White British (343969 samples)
##       - White British females and males (184688 and 159281 resp.)
...
## make separate lists of male/female IIDs
female_ids_df <- covars[covars$sex == 0, c('#FID', 'IID')]
fwrite(female_ids_df, file = '../data/sample-ids/filtered/female-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)

male_ids_df <- covars[covars$sex == 1, c('#FID', 'IID')]
fwrite(male_ids_df, file = '../data/sample-ids/filtered/male-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
...
## make list of White British male/female IIDs
wb_female_ids_df <- wb_ids_df[wb_ids_df$IID %in% female_ids_df$IID,]
fwrite(wb_female_ids_df, file = '../data/sample-ids/filtered/wb_female-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
 
wb_male_ids_df <- wb_ids_df[wb_ids_df$IID %in% male_ids_df$IID,]
fwrite(wb_male_ids_df, file = '../data/sample-ids/filtered/wb_male-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
...
ukb_rel <- fread('../data/relatedness/ukb_rel_a103076_s487957.dat', data.table = FALSE)
ukb_rel_2 <- ukb_rel[ukb_rel$Kinship >= 0.0884,]

## remove samples related (3rd degree or closer) with anyone in training set
...
fwrite(wb_ids_vali_df[wb_ids_vali_df$IID %in% female_ids_df$IID,],
       file = '../data/sample-ids/filtered/other_gb_vali_female-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
fwrite(wb_ids_vali_df[wb_ids_vali_df$IID %in% male_ids_df$IID,],
       file = '../data/sample-ids/filtered/other_gb_vali_male-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)

## remaining 9075 are test set
...
fwrite(wb_ids_test_df, file = '../data/sample-ids/filtered/other_gb_test_all-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
## male and female separately
fwrite(wb_ids_test_df[wb_ids_test_df$IID %in% female_ids_df$IID,],
       file = '../data/sample-ids/filtered/other_gb_test_female-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
fwrite(wb_ids_test_df[wb_ids_test_df$IID %in% male_ids_df$IID,],
       file = '../data/sample-ids/filtered/other_gb_test_male-ids.tab',
       sep = '\t', na = 'NA', quote = FALSE)
