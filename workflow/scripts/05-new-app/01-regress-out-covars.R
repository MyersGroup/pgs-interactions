...
## truncate (cap) very large residuals
lower_bd <- quantile(resid_df$res, 0.01)
upper_bd <- quantile(resid_df$res, 0.99)

resid_df_cap <- resid_df
resid_df_cap$res[resid_df_cap$res < lower_bd] <- lower_bd
resid_df_cap$res[resid_df_cap$res > upper_bd] <- upper_bd

## add IIDs and samples with missing values
resid_df_cap_all <- merge(phen_sp, resid_df_cap, by = "IID", all.x = TRUE)
resid_df_cap_all <- resid_df_cap_all[order(match(resid_df_cap_all$IID, sample_vec)),]  # same order as initially

resid_df_cap_all <- resid_df_cap_all[, c(1,3)]
resid_df_cap_all[, 2] <- round(resid_df_cap_all[, 2], digits = 8)

## add FIDs
resid_df_cap_all$FID <- resid_df_cap_all$IID
resid_df_cap_all <- resid_df_cap_all[, c("FID", "IID", "res")]
colnames(resid_df_cap_all) <- c("#FID", "IID", paste0(phen_name, ".res.cov"))

## export
fwrite(resid_df_cap_all,
       file = paste0("../results/05-new-app/residuals-covars/", phen, "/", phen, "-", samples, "-resid-covars.tab"),
...
fwrite(resid_df_qn_all,
       file = paste0("../results/05-new-app/residuals-covars/", phen, "_qn/", phen, "_qn-", samples, "-resid-covars.tab"),
       sep = "\t", na = "NA", quote = FALSE)
