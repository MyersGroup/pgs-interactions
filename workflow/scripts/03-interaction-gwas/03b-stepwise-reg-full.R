## Find independent hits from GWAS for each independent hit (full)
...
gwas <- fread(paste0("../results/03-interaction-gwas/indep-hits/", phen, "/snp-pgs/gwas/", phen, ".", train_sp, ".",
                     snp_, ".int.full.chr", snp_chr, ".", phen, ".res.cov.glm.linear.gz"), data.table = FALSE)
...
for (chr in 1:22) {
    ...
    gwas <- fread(paste0("../results/03-interaction-gwas/indep-hits/", phen, "/snp-pgs/gwas/", phen, ".", train_sp, ".",
                         snp_, ".int.full.chr", chr, ".", phen, ".res.cov.glm.linear.gz"), data.table = FALSE)
    colnames(gwas)[1] <- "CHR"
...
if (nrow(indep_hits_all) > 0) {
    ...
    fwrite(indep_hits_all,
           file = paste0("../results/03-interaction-gwas/indep-hits/", phen, "/snp-pgs/hits/", phen, "-", train_sp, "-full-", snp_, "-pairwise-hits.tab"),
           sep = "\t", na = "NA", quote = FALSE)

} else {

    file.create(paste0("../results/03-interaction-gwas/indep-hits/", phen, "/snp-pgs/hits/", phen, "-", train_sp, "-full-", snp_, "-pairwise-hits.tab"))
