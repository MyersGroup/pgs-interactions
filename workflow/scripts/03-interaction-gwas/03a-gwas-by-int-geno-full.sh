...
# Run interaction GWAS for full PGS independent hits
...
line=$(grep "${SNP//_/:}" ../results/03-interaction-gwas/indep-hits/${PHEN}/snp-pgs/hits/${PHEN}-${SAMPLES}-full-indep-hits.tab)
...
phen <- fread("../results/01-gwas/residuals-covars/${PHEN}/${PHEN}-${SAMPLES}-resid-covars.tab",
              data.table = FALSE)

geno <- fread("../results/03-interaction-gwas/indep-hits/${PHEN}/snp-pgs/gwas/${SNP}.geno.${CHR}.raw",
              data.table = FALSE)
geno <- left_join(geno, phen[, 2:3], by = "IID")
geno <- geno[!is.na(geno[["${PHEN}.res.cov"]]),]
...
then
    echo "Correlation between SNP genotype and its square is ($dom_cor)." > \
         ../results/03-interaction-gwas/indep-hits/${PHEN}/snp-pgs/gwas/${PHEN}.${SAMPLES}.${SNP}.int.full.snp_snp2_cor.log
fi
...
then
    ...
    $PLINK2 --pgen $PGEN \
            ...
            --parameters 1-4 \
            --vif 999 \
            --out ../results/03-interaction-gwas/indep-hits/${PHEN}/snp-pgs/gwas/${PHEN}.${SAMPLES}.${SNP}.int.full.chr${CHR}
...
gzip -f ../results/03-interaction-gwas/indep-hits/${PHEN}/snp-pgs/gwas/${PHEN}.${SAMPLES}.${SNP}.int.full.chr${CHR}.${PHEN}.res.cov.glm.linear
