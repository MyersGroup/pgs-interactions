...
phen_res_cov <- fread(paste0("../results/01-gwas/residuals-covars/", phen, "/", phen, "-", samples, "-resid-covars.tab"),
                      data.table = FALSE)
colnames(phen_res_cov)[3] <- "phen_res_cov"
phen_res_cov <- phen_res_cov[!is.na(phen_res_cov$phen_res_cov),]

## load full PGS
pgs_full <- fread(paste0("../results/02-pgs/iterative/", phen, "/r2_0.9-kb_500/wb_all/pgs/", phen, "-all-pgs-full.tab"),
                  data.table = FALSE)
## load LOCO PGS
pgs_loco <- fread(paste0("../results/02-pgs/iterative/", phen, "/r2_0.9-kb_500/wb_all/pgs/", phen, "-all-pgs-loco.tab"),
                  data.table = FALSE)
...
fwrite(pgs_full_mc_out,
       file = paste0("../results/02-pgs/mean-corrected/", phen,
